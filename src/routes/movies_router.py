...
@limiter.limit("100/minute")
async def get_movie_by_id(request: Request, movie_id: int, db: AsyncSession = Depends(get_db)):
    stmt = (
...
@limiter.limit("100/minute")
async def get_genre_list(request: Request, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Genre.id,
               Genre.name,
...
@router.get("/genres/{genre_id}/",
            ...
            status_code=status.HTTP_200_OK)
@limiter.limit("100/minute")
async def get_genre_detail(request: Request, genre_id: int, db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Genre)
...
@router.get("/stars/",
            ...
            status_code=status.HTTP_200_OK)
@limiter.limit("100/minute")
async def get_star_list(request: Request, db: AsyncSession = Depends(get_db)):
...
@router.get("/stars/{star_id}/",
            ...
            status_code=status.HTTP_200_OK)
@limiter.limit("100/minute")
async def get_star_with_movies(request: Request, star_id: int,
                               db: AsyncSession = Depends(get_db)):
    stmt = (
        select(Star)
...
@router.get("/directors/",
            ...
            status_code=status.HTTP_200_OK)
@limiter.limit("100/minute")
async def get_director_list(request: Request, db: AsyncSession = Depends(get_db)):
